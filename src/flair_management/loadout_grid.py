...
class Loadout_Grid:
    ...
    def fetch_loadout_grid(client):
        ...
        loadout_spray_uuids = [spray["SprayID"] for spray in loadout["Sprays"]]
        ...
        spray_index = 1
        for spray in sprays:
            if spray["uuid"] in loadout_spray_uuids:
                loadout_patched[f"Spray{spray_index}"] = {
                    "ID": spray["uuid"],
                    "type": "spray",
                    "display_name": spray["displayName"].replace("Spray", "").strip(),
                    "color": "White",
                }
                spray_index += 1
        ...
        loadout_patched["Title"] = {
            "ID": title_data["uuid"],
            "type": "title",
            "display_name": title_data["titleText"],
            "color": "White",
        }
        ...
        loadout_patched["Card"] = {
            "ID": card_data["uuid"],
            "type": "card",
            "display_name": card_data["displayName"],
            "color": "White",
        }
            ...
            for column,item_name in enumerate(items):
                ...
                if item_name is not None:
                    ...
                    if loadout_patched[item_name]["type"] == "weapon": 
                        ...
                        grid_built[row+1].append((weapon_data["color"],f"{weapon_data['level_name']}\t".expandtabs(longest)))
                        grid_built[row+2].append((weapon_data["color"],f"{weapon_data['chroma_name']}\t".expandtabs(longest)))
                        grid_built[row+3].append((f"{weapon_data['color']} italic",f"{weapon_data['buddy_name']}\t".expandtabs(longest)))
